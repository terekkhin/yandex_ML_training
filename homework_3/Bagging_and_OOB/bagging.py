...
class SimplifiedBaggingRegressor:
    ...
    def predict(self, data):
        ...
        for j in range(self.num_bags):
            preds += self.models_list[j].predict(data)
    ...
    def _get_averaged_oob_predictions(self):
        ...
        self.oob_predictions = [np.mean(self.list_of_predictions_lists[i]) if self.list_of_predictions_lists[i] else
                                np.nan for i in range(len(self.data))]# Your Code Here
    ...
    def OOB_score(self):
        ...
        res = (np.array(self.oob_predictions) - self.target)
        res = res[~np.isnan(res)]
        return np.mean(np.square(res))# Your Code Here